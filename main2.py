...
print("SBER.ME, AAPL, BTC-RUB, DOGE-RUB,LTC-RUB,RUB=X")
while True:
  time.sleep(0.04)
  ...
  import Bwork_voice
  print(Bwork_voice.speek_text(data))
  print(data.values)
  print(data.shape)
  print(data)
  ...
  data_btc_usd = yf.download(inspector, start="1000-01-01", end=real_date, interval='1d')
  print(data_btc_usd)
  ...
  model.fit(x_train, y_train, batch_size=2, epochs=999)
  ...
  answer = "Прогнозируемый результат " +str(inspector)+ str(pred_price) + "\n\nВремя обработки запроса сек.: " + str(
      int(end_timer))
